...
export const AddPrice = ({isAdmin}) => {
    ...
    const getData = () => {
        ...
        axios.get('https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Price', { cancelToken: source.token })
        .then((response) => {
            setIsPending(false)
    ...
    const deletePrice = MyPrice => {
            if(window.confirm(`Ви впевнені, що хочете видалити послугу: ${MyPrice.title}?`)) {
                setIsPending(true)
                axios.delete(`https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Price/${MyPrice.id}`)
                    .then(() => {
                        getData()
                    })
                    . catch((err) => {
    ...
    const addNewPrice = (MyPrice) => {
        setIsPending(true)
        axios.post('https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Price', MyPrice)
            .then((response) => {
                getData()
            })
            .catch((err) => {
    ...
    const EditPrice = (updatePrice) => {
        setIsPending(true)
        axios.put(`https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Price/${updatePrice.id}`, updatePrice)
            .then(() => {
                getData()
            })
    }
            ...
            if(PriceArr.length === 0)
                return <div className='waitDownl'><h1>Завантаження даних...</h1></div> 
                    ...
                    <div className='containerVac'>
                        ...
                        <div className='price' style={{opacity: vacOpacity}}>
                            <div className='vacList'>
                                <div className='vacName' style={{fontWeight: '1000', color: '#000'}}>Назва послуги</div>
                                <div className='PoslugaPrice' style={{fontWeight: '1000', color: '#000'}}>Ціна, грн</div>
                            </div>
                            <div className='price'>
                                {pricePosts}
                            </div>
...

                        {
                            ...
                            <div className='addVacButton'>
                                <button onClick={handleShowAddPriceForm}>Додати нову послугу</button>
                            </div>
                        }
                    </div>
                </>
