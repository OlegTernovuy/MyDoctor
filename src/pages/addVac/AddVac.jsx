...
export const AddVac = ({isAdmin}) => {
    ...
    const getData = () => {
        ...
        axios.get('https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Vacs', { cancelToken: source.token })
        .then((response) => {
            setIsPending(false)
    ...
    const deleteVac = MyVac => {
            if(window.confirm(`Ви впевнені, що хочете видалити вакцину: ${MyVac.title}?`)) {
                setIsPending(true)
                axios.delete(`https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Vacs/${MyVac.id}`)
                    .then(() => {
                        getData()
                    })
                    . catch((err) => {
    ...
    const addNewVac = (MyVac) => {
        setIsPending(true)
        axios.post('https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Vacs', MyVac)
            .then((response) => {
                getData()
            })
            .catch((err) => {
    ...
    const EditVac = (updateVac) => {
        setIsPending(true)
        axios.put(`https://62e3d9aa3c89b95396d1ebbd.mockapi.io/Vacs/${updateVac.id}`, updateVac)
            .then(() => {
                getData()
            })
    }
                    ...
                    <div className='containerVac'>
                        ...
                        <div className='price' style={{opacity: vacOpacity}}>
                            <div className='vacList'>
                                <div className='vacName' style={{fontWeight: '1000', color: '#000'}}>Назва вакцини</div>
